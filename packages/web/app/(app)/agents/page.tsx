...
type Agent = { id: number; name: string; instruction: string; schedule: string; is_active: boolean; delivery?: string; last_run?: string | null; model?: string };
...
export default function AgentsShell() {
  ...
  function openEdit(a: Agent) { setForm({ id: a.id, name: a.name, instruction: a.instruction, schedule: a.schedule, delivery: a.delivery || 'chat', model: a.model }); setShowForm(true); }
  ...
  return (
    <div className="p-6 space-y-3">
      <div className="flex gap-2">
        <button className="px-3 py-1.5 bg-white text-black rounded" onClick={openNew}>New Agent</button>
      </div>
      <div className="flex gap-2">
        {tabs.map((t) => (
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4">
          <div className="w-full max-w-xl bg-black border border-white/10 rounded p-4 space-y-3">
            <div className="text-lg font-semibold">{form.id ? 'Edit Agent' : 'New Agent'}</div>
