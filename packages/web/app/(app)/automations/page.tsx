...
export default function AutomationsShell() {
  ...
  async function save() {
    if (form.id) {
      await fetch(`${CORE_URL}/api/automations/${form.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: form.name, instruction: form.instruction, schedule: form.schedule, delivery_method: form.delivery, model: form.model }) });
    } else {
      await fetch(`${CORE_URL}/api/automations`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: form.name, instruction: form.instruction, schedule: form.schedule, delivery_method: form.delivery, model: form.model }) });
    }
    setShowForm(false);
    await refresh();
  }
  ...
  async function toggle(a: Automation) {
    await fetch(`${CORE_URL}/api/automations/${a.id}/toggle`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_active: !a.is_active }) });
    await refresh();
  }

  async function runNow(a: Automation) {
    await fetch(`${CORE_URL}/api/automations/${a.id}/run`, { method: 'POST' });
  }
